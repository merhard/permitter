...
For example:
  ...
  class ProjectController < ApplicationController
    ...
    def index
      @projects = Project.permitted_by(current_permissions, :edit)
    end

  end

The +@projects+ variable will now be scoped similar to +Project.where(user_id: current_user.id)+.


If no currently used permissions match the scoping required, use +:permitted_by+ as the action in +Permission+.
  ...
  class Permission
    ...
    def initialize(user)
      ...
      if user
        ...
        allow_action :projects, :permitted_by do |project|
          # your scope here
        end
      end
    end
...
Note that when :permitted_by is used, action is now optional.


When writing scopes via an association, :permitted_by must be used.
